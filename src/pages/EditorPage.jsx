...
const EditorPage = () => {
  ...
  const handleDownloadPDF = () => {
    if (!isPro) {
      ...
      downloadPDF(cvData, selectedTemplate, userPhoto, t);
    }
  };
    ...
    <div className="h-screen flex flex-col md:flex-row font-sans text-slate-800 bg-slate-50 relative overflow-hidden">
      ...
      >
        ...
        <div className="sticky top-0 bg-white/95 backdrop-blur z-20 p-4 border-b border-slate-100 shadow-sm">
          ...
          <div className="flex gap-2 overflow-x-auto p-2 -mx-2 scrollbar-hide">
            {tabs.map(tab => (
              <button
                key={tab.id}
        ...
        <div className="p-4 border-t border-slate-200 bg-white z-10 space-y-3 relative mt-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2 flex justify-center gap-2 pb-2">
              {['modern', 'creative'].map((type) => (
                <button
                  key={type}
                  onClick={() => setSelectedTemplate(type)}
                  className={`px-3 py-1 rounded-full text-xs font-bold capitalize border ${selectedTemplate === type ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-500 border-slate-200'}`}
                >
                  {t[type]}
                </button>
              ))}
        ...
        <div className="w-full flex-1 flex items-start overflow-auto pb-20">
          ...
          <div className="m-auto transition-all duration-200 ease-out" style={{ width: `${816 * zoom}px`, height: `${1056 * zoom}px` }}> {/* 8.5in * 96dpi = 816px, 11in = 1056px */}
            <div
              ...
              className="min-w-[8.5in] min-h-[11in] bg-white shadow-2xl"
            >
              {selectedTemplate === 'modern' ? (
                <ModernTemplate data={cvData} labels={t} />
              ) : (
                <CreativeTemplate data={cvData} photo={userPhoto} labels={t} />
              )}
            </div>
          </div>
        </div>
